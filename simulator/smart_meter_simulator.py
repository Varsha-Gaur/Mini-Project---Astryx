...
--------------------------
*  Removed hard dependency on ``kafka_producer`` (module not in project).
*  ``dataset_path`` now has a default so the class can be instantiated
   without a file path for purely synthetic runs.
*  ``collect_encrypted_batch`` now correctly stores the returned
   ``EncryptedReading`` objects (the original discarded them).
